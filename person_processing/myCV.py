...
class Net():
    ...
    def process(self, img):
        blob = cv2.dnn.blobFromImage(img, self.scale, self.size)
...
def detect(net: Net, img, trigger: float, ID: int = 1):
    objects = {}
    out = net.process(img)
...
def get_distance(descriptor_1, descriptor_2):
    out = np.dot(descriptor_1, descriptor_2)

    return out


def object_in_data(descriptor, data: dict, distance_trigger: float):
    max_distance = 0
    ID = 0
    if len(data) != 0:
        for x in data:
